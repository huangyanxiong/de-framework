...
/**
 * Decode and encode between list of zNode value strings and list of {@link T}s.
 */
public interface NodeValueCodec<T> {
  ...
  /**
   * Decode a zNode value into a set of {@link T}s.
   *
  ...
  /**
   * Decode a zNode value to a {@link T}.
  ...
  /**
   * Encode a set of {@link T}s into a bytes array representation
   * @param entries set of {@link T}s
   * @return a bytes array
   */
  byte[] encodeAll(Iterable<T> entries);

  /**
   * Encode single {@link T} into a bytes array representation
   * @param entry  an {@link T}
   * @return a bytes array
   */
  byte[] encode(T entry);
}